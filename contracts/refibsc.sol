...
/// @custom:security-contact security@reimagined.fi
...
contract REFI is Pausable, Ownable, IERC20 {
    address constant UNISWAPROUTER =
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO = 0x0000000000000000000000000000000000000000;

    string private constant _name = "ReFi";
    string private constant _symbol = "REFI";

    uint256 public buyTreasuryFeeBPS = 1198;
    uint256 public buyLiquidityFeeBPS = 1;
    uint256 public buyRewardFeeBPS = 1;
    uint256 public buyTotalFeeBPS = 1200;

    uint256 public sellTreasuryFeeBPS = 1198;
    uint256 public sellLiquidityFeeBPS = 1;
    uint256 public sellRewardFeeBPS = 1;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[recipient] && sellTotalFeeBPS > 0) {
                ...
                tokensForTreasury += (fees * sellTreasuryFeeBPS) / sellTotalFeeBPS;
                tokensForRewards += (fees * sellRewardFeeBPS) / sellTotalFeeBPS;
                tokensForLiquidity += (fees * sellLiquidityFeeBPS) / sellTotalFeeBPS;
            } else if (
                automatedMarketMakerPairs[sender] && buyTotalFeeBPS > 0
            ) {
                // buying
                fees = (amount * buyTotalFeeBPS) / 10000;
                tokensForTreasury += (fees * buyTreasuryFeeBPS) / buyTotalFeeBPS;
                tokensForRewards += (fees * buyRewardFeeBPS) / buyTotalFeeBPS;
                tokensForLiquidity += (fees * buyLiquidityFeeBPS) / buyTotalFeeBPS;
            }
            amount -= fees;
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        uint256 swapTokensTotal = swapTokensRewards + swapTokensTreasury + swapTokensLiquidity;
        ...
        uint256 nativeRewards = (nativeSwapped * swapTokensRewards) / swapTokensTotal;
        uint256 nativeLiquidity = nativeSwapped - nativeTreasury - nativeRewards;
...
contract RewardTracker is Ownable, IERC20 {
    ...
    string private constant _name = "REFI_RewardTracker";
    string private constant _symbol = "REFI_RewardTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "REFI_RewardTracker: account already set to requested state"
        );
        excludedFromRewards[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "REFI_RewardTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "REFI_RewardTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "REFI_RewardTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("REFI_RewardTracker: method not implemented");
    }
