...
contract LaunchLinearVestingFactory is AccessControlEnumerable, Initializable {
    ...
    event PushLaunchLinearVestingVersion(ILaunchLinearVesting indexed newLaunch, uint256 newVersionId);
    event SetLaunchLinearVestingVersion(uint256 previousVersionId, uint256 newVersionId);
    event UpdateLaunchAdmins(address previousLaunchProxyAdmin, address indexed newLaunchProxyAdmin, address indexed previousLaunchAdmin, address indexed newLaunchAdmin);
    ...
    function initialize(
        ...
        require(_launchProxyAdmin != _launchAdmin, 'launchProxyAdmin and launchAdmin cannot be the same');
    ...
    function activeImplementationContract() public view returns (ILaunchLinearVesting) {
    ...
    function pushImplementationContract(ILaunchLinearVesting _newImplementation) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint32 size;
        assembly {
    ...
    /// @notice Add and use new implementation
    function _pushImplementationContract(ILaunchLinearVesting _newImplementation) internal {
        LaunchLinearVestingImplementations.push(_newImplementation);
        LaunchLinearVestingVersion = LaunchLinearVestingImplementations.length - 1;
        emit PushLaunchLinearVestingVersion(_newImplementation, LaunchLinearVestingVersion);
    }
    ...
    function setImplementationContract(uint256 _index) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_index < LaunchLinearVestingImplementations.length, "version out of bounds");
    ...
    function setLaunchAdmins(address _newLaunchProxyAdmin, address _newLaunchAdmin) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_newLaunchProxyAdmin != _newLaunchAdmin, 'launchProxyAdmin and launchAdmin cannot be the same');
        emit UpdateLaunchAdmins(launchProxyAdmin, _newLaunchProxyAdmin, launchAdmin, _newLaunchAdmin);
